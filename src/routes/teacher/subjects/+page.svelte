<script lang="ts">
  ...
  import { Loader2, Plus, Pencil, Trash2, X, BookOpen, ListTree } from 'lucide-svelte';

  const isAdmin = $derived($user?.role === 'ADMIN');

  // Admin (v1): subject grid dengan topik bersarang.
  ...
  // Teacher (v2): grid topik agregat lintas mapel diampu.
  interface TopicItem {
    id: string;
    name: string;
    subjectId: string;
  ...
  async function load() {
    ...
    try {
      ...
      subjects = ((subjRes as any).data ?? []) as Subject[];
  ...
  // ===== Topic CRUD =====
  // Admin: per-subject (tanpa pilih mapel). Teacher: pilih mapel diampu.
  function openTopic(subjectId: string) {
    editingTopic = null;
    topicSubjectId = subjectId;
    topicName = ''; topicErr = ''; showTopic = true;
  }
  ...
  function openEditTopic(t: TopicItem) {
    ...
    topicName = t.name; topicErr = ''; showTopic = true;
  }
  ...
  async function delTopic(t: TopicItem | Topic) {
            ...
            {#each s.topics as t (t.id)}
              <div class="flex items-center justify-between rounded-lg bg-accent/60 px-3 py-1.5 text-sm">
                ...
                <div class="flex items-center gap-1">
                  ...
                  <button onclick={() => delTopic(t)} class="text-rose-600"><Trash2 class="h-3.5 w-3.5" /></button>
                </div>
              </div>
            {/each}
          </div>
